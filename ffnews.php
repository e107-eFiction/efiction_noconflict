...
if($nid) {
	...
	if($ccount) {
		...
		while($comments = dbassoc($query2)) {
			...
			if(isADMIN && uLEVEL < 4)
				$c->assign("adminoptions", "<div class='adminoptions'><span class='label'>"._ADMINOPTIONS.":</span> [<a href=\"ffnews.php?action=newsstory&amp;edit=".$comments['cid']."&amp;nid=$nid\">"._EDIT."</a>] [<a href=\"ffnews.php?action=newsstory&amp;cid=".$comments['cid']."&amp;del=1&amp;nid=$nid\">"._DELETE."</a>]</div>");
			$c->assign("oddeven", ($count % 2 ? "odd" : "even"));
			$count++;
		}
		$output .= $c->getOutputContent( );
		if($ccount > $itemsperpage) $output .= build_pagelinks("ffnews.php?nid=$nid&amp;", $ccount, $offset);
	}
	if(isMEMBER) {
		$output .= "<form method=\"POST\" enctype=\"multipart/form-data\" action=\"ffnews.php?action=newsstory&amp;nid=$nid\">";
	...
	$output .= $news->getOutputContent( );
	if ($count > $itemsperpage) $output .= build_pagelinks("ffnews.php?", $count, $offset);
	$tpl->assign("output", $output);
}
