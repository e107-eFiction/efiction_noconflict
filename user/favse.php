...
if(empty($favorites)) accessDenied( );
	...
	if($delete) {
		...
		$tpl->assign("pagelinks", "<form method=\"POST\" enctype=\"multipart/form-data\" action=\"author.php?action=favse&amp;add=1&amp;seriesid=".($add ? $add : $edit)."\">\n
	...
	if($seriescount) {
		...
		while($stories = dbassoc($list)) { 
			...
			if(USERUID == $uid) 
				$cmt->assign("commentoptions", "<div class='adminoptions'><span class='label'>"._OPTIONS.":</span> <a href=\"author.php?action=favse&amp;edit=".$stories['seriesid']."\">"._EDIT."</a> | <a href=\"author.php?action=favse&amp;delete=".$stories['seriesid']."\">"._REMOVEFAV."</a></div>");
